...
let rec flatten_doc #a (d: doc a): flat_doc a
  ...
  | DIndent idoc ->  
	   ...
	   let f (x: flat_annot a {memP x fan}): (r: flat_annot a {Annot?.pend r < length fdoc'}) = 
	       let Annot s e a = x in Annot (map_pos s) (map_pos e) a
	   in fdoc', map' f fan
  | DConcat d1 d2 -> 
...
// let doc_to_lookup_table #a (d: doc a)
//   = flat_doc_to_lookup_table (flatten_doc d)


let writeToFile file content
   = let _ = Tactics.launch_process "sh" ["-c"; "cat - > " ^ file] content in ()


let test = indent (
   nl <+> str "salut" <+> space <+> annote (str "gros") "gros" <+> space <+> str "hoidhaosdj"
   <+> nl <+>
   str "salut" <+> space <+> annote (str "gros") "gros"
)

let _ = Tactics.run_tactic (fun _ ->
  let str = doc_to_string test in
  let ann = doc_to_annotations test in
 let l = map (fun (s, e, a) -> Doc.string_of_position s ^ " .. "
                          ^ Doc.string_of_position e ^ " ==> " ^ a ) ann in
  writeToFile "out.x" (str ^ "\n\n\n\n\n" ^ 
  	 String.concat "\n" l
  )
)
